...
**Offline readiness status:** Issue #357 assembles a canonical bounded v2 package over the complete Stage 4 source closure plus exact chart, synthetic values, freshly reproduced local render, trusted render receipt, placeholder-image record, active NIC v2 contract, runtime pins, authenticated-runtime evidence, Stage 4/5 schema inventory, and local-validation digests. A bounded trusted preparation executable authenticates the chart/values and pinned Helm executable identity/version, runs local `helm template` twice, and compares fresh output to committed bytes; the classifier remains pure. Its `local_preparation_complete=true` and authenticated candidate-artifact claims are local/static only, not runtime observation or campaign readiness. Containerd `2.2.1` and Kata-bundled QEMU `11.0.1` artifact identities are authenticated; QEMU `8.2.2` is historical only. Release worker/sandbox images remain `.invalid` placeholders, and the region-specific EKS AMI/running kernel and selected runtime remain unobserved. NIC v2 resolves only external launch-template selection capability; launch-template contents remain unobserved. Historical readiness/NIC v1 artifacts remain preserved. ADR 0094 adds a separate bounded local static manifest/NIC handoff while preserving Helm's zero-manifest boundary. Active blockers remove only historical NIC capability and containerd/QEMU artifact identity; issue #42, AWS image/kernel, account, price, quota, identities, approval/envelope, release images, and provider route remain. Campaign request readiness, approval, cloud authority/execution, provider truth, current/zero resources, Stage 4 exit, and release eligibility are false. The closed resource graph, spend/TTL, synthetic-data, stop/destroy, and service-specific inventory controls are proposals only. Any approval is one-attempt-only and never authorizes retry. Fresh revalidation covers #42 closure plus source, pin, price/quota, graph, Helm/chart/values/renderer/toolchain, validation/advisory expiry, account/principal/separation, approval/envelope/attempt, destroy, and inventory changes.
