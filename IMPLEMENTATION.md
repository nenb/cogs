...
- constructs `ModelRuntime` with application-owned in-memory credential storage and an exact API-key-provider allowlist, plus `SessionManager` and the resource loader explicitly;
...
- Construct `ModelRuntime` with application-owned in-memory credential storage and an exact API-key-provider allowlist, plus `SessionManager` and `createAgentSession()` explicitly.
...
**Static storage/launch status:** Issue #355 adds strict local-only storage roles and one-session object-graph classification. It fixes a 20 GiB CSI-block `Filesystem` / `ReadWriteOncePod` / `WaitForFirstConsumer` / `Retain` sandbox workspace retained until explicit deletion, a distinct 5 GiB `Filesystem` / `ReadWriteOncePod` / `WaitForFirstConsumer` / `Retain` trusted Pi state volume retained for 30 days, a fenced single-writer lease, exactly one worker/proxy resource and one Kata sandbox bound to classifier-derived domain-separated immutable metadata/digest and exact session/workspace/resource references, a priori host-key match, out-of-band ephemeral identities, and sticky uncertainty-preserving cleanup that carries state/resources/attachments/lease. The classifier launches and observes nothing; RuntimeClass presence is only a caller assertion and every provider/runtime claim remains false. The historical NIC v1 missing-capability assessment remains preserved, while active v2 resolves only personal-fork source capability with operator attestation and no provider observation.

**Offline readiness status:** Issue #357 and follow-ups #390 and ADR 0098 assemble a canonical bounded v4 package over the complete current Stage 4 readiness-source closure plus exact chart, synthetic values, freshly reproduced local render, trusted render receipt, active NIC v2 contract, runtime pins, authenticated-runtime evidence, Stage 4/5 schema inventory, and local-validation digests. The v4 image lock preserves protected-main publication assertion SHA-256 `ad45d6b0a114c481eb869daff38f4ddc669801e8a5b5d808c404b506ae33c450`, independent review SHA-256 `f6a607349062f5ca9211978600012b4cd16651af984c083a0329646a74da6a3a`, workflow run `30852317459`, image-source revision `d3ddb987ceeec0bae0fa2d89fdc134187a0d1de3`, and exact worker/sandbox GHCR digests as historical records only. The Pi 0.84.2 dependency closure postdates those images, so no current-source release image set is present. A bounded trusted preparation executable authenticates the chart/values and pinned Helm executable identity/version, runs local `helm template` twice, and compares fresh output to committed bytes; the classifier remains pure. Its `local_preparation_complete=true` is a local record only; `release_image_set_present=false` and static image identity closure is false, with no runtime observation or campaign readiness. Containerd `2.2.1` and Kata-bundled QEMU `11.0.1` artifact identities are authenticated; QEMU `8.2.2` is historical only. The region-specific EKS AMI/running kernel and composed runtime remain unobserved. NIC v2 resolves only external launch-template selection capability; launch-template contents remain unobserved. Historical readiness v1/v2/v3 and NIC v1 artifacts remain preserved. ADR 0094 keeps a separate bounded local static manifest/NIC handoff while preserving Helm's zero-manifest boundary. Active blockers remove historical NIC capability and containerd/QEMU artifact identity blockers; `RELEASE_IMAGE_SET_ABSENT`, issue #42, AWS image/kernel, account, price, quota, identities, approval/envelope, and provider route remain. Campaign request readiness, approval, cloud authority/execution, provider truth, current/zero resources, exact image runtime closure, Stage 4 exit, production readiness, and release eligibility are false. The closed resource graph, spend/TTL, synthetic-data, stop/destroy, and service-specific inventory controls are proposals only. Any approval is one-attempt-only and never authorizes retry. Fresh revalidation covers #42 closure plus source, pin, price/quota, graph, Helm/chart/values/renderer/toolchain, validation/advisory expiry, account/principal/separation, approval/envelope/attempt, destroy, and inventory changes.
...
The historical v1 contract remains at [`deploy/nic/stage4-sandbox-node-group-contract-v1.json`](deploy/nic/stage4-sandbox-node-group-contract-v1.json) and retains the exact NIC `v0.11.0` / module `0.7.0` `blocked-missing-capability` result. The active v2 contract is [`deploy/nic/stage4-sandbox-node-group-contract.json`](deploy/nic/stage4-sandbox-node-group-contract.json), documented in [`docs/operations/stage-4-nic-node-group-contract.md`](docs/operations/stage-4-nic-node-group-contract.md). It pins explicitly accepted personal-fork NIC commit `53b1a791ed1ff394969e0aeaa6379be955244b62`, tree `32c14bd9a19c0519006a9b86284402f9e0187947`, and module commit `c3017c0e15b538cd4e04c0786809a861ea82c621`, tree `59105ac6f037977d0dddebf844affa06e0b01236`, with exact reviewed blob/content/size closure. The v2 classifier resolves only source-level external launch-template ID/version preservation with operator attestation. It fixes launch-template contents, provider truth, cloud execution, and campaign authority to unobserved/false. A public EKS node-image release candidate is pinned, while its region-specific AMI ID and running kernel remain unresolved.

The active contract expresses this required static shape:
...
- Kata `3.32.0` archive SHA-256 `1449ecea50bd91fa73a94648db195d18950fe869ba4b1f12d05f55f1fa7c1b01`, containerd `2.2.1`, active Kata-bundled QEMU `11.0.1`, `io.containerd.kata.v2`, and KVM-only acceleration (`8.2.2` is historical host context only);
- no trusted platform workload scheduling on sandbox nodes and no runc/TCG fallback.

Keep trusted Cogs/proxy resources on ordinary nodes selected by `cogs.dev/node-domain=trusted`, with no sandbox toleration. V1 remains historical and has no ready state. V2's `source-capability-satisfied-local-static` is not a ready or observed state: actual launch-template contents/preservation, AMI/KVM state, runtime identity, and scheduler behavior require separately authorized future evidence. ADR 0094 separately permits deterministic local manifest and NIC handoff bytes while retaining zero deployment authority.
