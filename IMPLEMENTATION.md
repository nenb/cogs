...
**Offline readiness status:** Issue #357 assembles a canonical bounded package over exact source, chart, synthetic values, byte-identical local render, image-lock, NIC, runtime-pin, Stage 4/5 schema, and local-validation digests. Its pure classifier may return `local_preparation_complete=true` only for the exact locally consistent package, while fixing campaign request readiness, approval, cloud authority/execution, provider truth, current/zero resources, Stage 4 exit, and release eligibility to false. The proposed account binding and separated campaign identities are absent; current price/quota are undiscovered; NIC `v0.11.0` / module `0.7.0` remains blocked and the EKS AMI/image/kernel remains unresolved. The proposed `us-east-1` / `c8i-flex.large` graph, one-sandbox-node ceiling, USD 20 spend cap, 5,400-second EKS duration, 14,400-second TTL, synthetic data, SSM-only access, stop/destroy flow, and independent inventory scopes are proposals only. There is no executable provider route. Any future approval is one-attempt-only, never authorizes retry, and requires fresh revalidation after #42 closure or any source, pin, current-price, current-quota, or campaign-shape change.
