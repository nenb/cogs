...
class FixedProvider:
    ...
    def remote(self, ordinal: int, mode: str, grant_commitment: str,
        ...
        remote_shell = (
            ...
            f"$g -C \"$w/H\" fetch -q --no-tags --depth=1 origin {self.approval.implementation_revision}; "
            "$g -C \"$w/H\" checkout -q --detach FETCH_HEAD; umask 077; "
            "$g init -q \"$w/Q\"; $g -C \"$w/Q\" remote add origin https://github.com/nenb/cogs.git; "
            f"$g -C \"$w/Q\" fetch -q --no-tags --depth=1 origin {self.approval.qualification_revision}; "
            "$g -C \"$w/Q\" checkout -q --detach FETCH_HEAD; "
