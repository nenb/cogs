...
def _seccomp_program() -> tuple[tuple[int, int, int, int], ...]:
    ...
    # Shared audit arch: reject every x32-tagged nr before native dispatch.
    # Bare 512–547 remain ALLOW; this does not establish pre-5.4 kernel safety.
    rows = [ (0x20, 0, 0, 4), (0x15, 1, 0, 0xC000003E), (0x06, 0, 0, 0x80000000), (0x20, 0, 0, 0), (0x45, 0, 1, 0x40000000), (0x06, 0, 0, deny), (0x15, 0, 10, 322), (0x20, 0, 0, 16), (0x15, 0, 6, 198), (0x20, 0, 0, 20), (0x15, 0, 4, 0), (0x20, 0, 0, 48), (0x15, 0, 2, _AT_EMPTY_PATH), (0x20, 0, 0, 52), (0x15, 1, 0, 0), (0x06, 0, 0, deny), (0x06, 0, 0, 0x7FFF0000), (0x15, 0, 4, 157), (0x20, 0, 0, 16), (0x15, 1, 0, _PR_GET_SECCOMP), (0x06, 0, 0, deny), (0x06, 0, 0, 0x7FFF0000), (0x20, 0, 0, 0), ]
...
class _SystemOps:
    ...
    def open(self, path: str, flags: int, mode: int = 0o600) -> int:
        ...
        code = self._open_enoent_code(path)
        raise RuntimeLauncherError("required launcher object absent", code) from None
...
def _namespace_owner(
    ...
    try:
        ...
        child_owner.plan_exec(child_lease, planned_executable)
        ...
        sequence = 5
        final_inspection = _final_child_inspection(ops, child, mapping_identities, role, report, planned_executable, post_inspection, namespace_handles, owner_limits_sha256, set_permission_stage)
...
def _mapping_inspection(ops: _SystemOps, pid: int, authority: dict[tuple[str, str], tuple[int, int, int]], role: str, report: dict[str, object], planned_executable: tuple[int, int], first_snapshot: bytes | None = None) -> tuple[bytes, dict[str, object]]:
    ...
    for _start, _end, permissions, _offset, major, minor, inode, path in _parse_maps(first):
        ...
        if inode == 0:
            _require(path in (b"[vdso]", b"[vsyscall]"), "unknown executable synthetic mapping", "mapping-unknown")
            continue
        key = reverse.get((major, minor, inode))
        _require(key is not None, "unknown executable mapping identity", "mapping-unknown")
        _require(key not in observed, "duplicate executable mapping identity", "mapping-duplicate")
        observed.append(key)
