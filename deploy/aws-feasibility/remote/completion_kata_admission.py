...
def _validate_contract(raw, expected):
    ...
    _require(len({path for (_kind, path) in identities}) == len(identities), 'tool closure aliases an object path'); _require(total <= 512 * 1024 * 1024 and identities[0] == ('executable', expected['path']) and (sum((kind == 'executable' for (kind, _path) in identities)) == 1) and (sum((kind == 'loader' for (kind, _path) in identities)) <= 1)); _require(objects[0]['sha256'] == expected['executable_sha256']); loader_count = sum((kind == 'loader' for (kind, _path) in identities)); libraries = objects[1 + loader_count:]; library_sonames = {item['soname'] for item in libraries}; _require([item['soname'] for item in libraries] == sorted(library_sonames) and (loader_count == 1) == bool(libraries)); _require(all((name in library_sonames for item in objects for name in item['needed']))); by_soname = {item['soname']: item for item in libraries}; (pending, reached) = (list(objects[0]['needed']), set())
    while pending:
        name = pending.pop(0)
        if name in reached:
            continue
