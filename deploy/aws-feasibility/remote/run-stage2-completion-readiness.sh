#!/bin/sh
# Sole zero-argument sealed marker-only readiness-cycle owner entry.
set -eu
[ "$#" -eq 0 ] || exit 64
...
exec /usr/bin/env -i HOME=/nonexistent LANG=C LC_ALL=C \
  ...
  /usr/bin/python3 -I -B \
  /var/lib/cogs/stage2-completion-v1/source/deploy/aws-feasibility/remote/completion_cycle_readiness.py
