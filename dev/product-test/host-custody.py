...
def environment(config, role=None):
    ...
    require(isinstance(values, list) and len(values) == len(set(v.split("=", 1)[0] for v in values)))
    for value in values:
        key, sep, text = value.partition("=")
        require(sep and key in allowed and text == allowed[key])
...
def inventory(fd, prefix="", result=None, markers=None):
    ...
    for name in sorted(os.listdir(fd)):
        ...
        path = prefix + name
        require(len(result) < 512 and path.count("/") < 16)
...
def unique_object(pairs):
    value = dict(pairs)
    require(len(value) == len(pairs) and "__proto__" not in value)
    return value
...
class Custody:
    def __init__(self, config):
        ...
        self.publication, self.history, self.shutdown = None, None, False
        self.admissions, self.nodes, self.publications, self.records = [], 0, [], set()
        self.selector = selectors.DefaultSelector()
        self.recovery = config.get("cleanup_only") is True
        ...
        try:
            ...
            os.fsync(parent)
            self.fd = directory(parent, self.generation, None if self.recovery else 0o700)
            ...
            if self.recovery:
                self.reopen()
                return
            self.record("intent", config)
            self.record("root", identity(os.fstat(self.fd))[:2])
    ...
    def saved(self, name):
        s = os.stat(name, dir_fd=self.control, follow_symlinks=False)
        require(s.st_uid == 0 and stat.S_IMODE(s.st_mode) == 0o400)
        raw = capture(self.control, name)
    ...
    def rollback(self):
        ...
        if self.control is not None:
            self.record("cleanup-required", {"generation": self.generation})
            self.settle()
        elif self.lock is not None:
            data = canonical({"generation": self.generation, "cleanup_required": True})
            os.lseek(self.lock, 0, os.SEEK_END)
            require(os.write(self.lock, data) == len(data))
    ...
    def reopen(self):
        ...
        for role in ("trust", "sandbox", "worker"):
            if role + "-intent" in os.listdir(self.control) and role + "-retired" not in os.listdir(self.control):
                ...
                self.ids[role] = self.saved(role + "-receipt")
                image = self.ids[role]["spec"]["image"]
                self.images[image] = self.saved("image-" + image[7:])
        for name in os.listdir(self.control):
            if name.startswith("seal-"):
                seal = self.saved(name)
                fd = os.open(self.root + "/" + seal["path"], os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW)
                require(list(identity(os.fstat(fd))[:2]) == seal["identity"])
                self.sealed.append((fd, seal["files"]))
    ...
    def authenticate(self, role):
        ...
        require(environment(v["Config"], role) == held["environment"])
        h = v["HostConfig"]
    ...
    def bind_receipt(self):
        ...
        for scope in ("shared", "user"):
            ...
            bundle = next(iter(entries)).split("/")[0]
            expected = {**self.publication["sources"][scope], "destination": f"/{scope}/skills/{bundle}",
                        "read_only": True, "bundle_digest": "sha256:" + bundle}
            require(r[scope] == expected and type(r[scope]["read_only"]) is bool)
            source = f"{self.root}/publication/{self.generation}/{scope}/{bundle}"
    ...
    def storage_observation(self, name):
        ...
        backing = self.saved(name + "-backing")
        ...
        loops = [v for v in loops if v["back-file"] == path]
        require(len(loops) <= 1)
    ...
    def provenance(self, q):
        ...
        git = lambda *a: self.command(["git", "-C", root, *a])
        require(git("rev-parse", "HEAD").decode().strip() == q["candidate"])
        require(not git("status", "--porcelain=v1", "--untracked-files=all"))
        require(not git("diff", q["baseline"], "--", "images/sandbox", "images/worker", "package-lock.json"))
        source = {}
        ...
        stock = self.images[q["stock_worker_image"]]["RootFS"]["Layers"]
        layers = self.images[q["worker_image"]]["RootFS"]["Layers"]
        ...
        self.record("provenance", receipt)
        self.source = source
        return receipt
    ...
    def verify_candidate(self, cid):
        ...
        for path in ("src", "schemas", "dev/product-test", "dev/launcher/api-client.ts", "third_party"):
            data = self.docker("cp", cid + ":/opt/cogs/" + path, "-")
            observed = {}
            with tarfile.open(fileobj=io.BytesIO(data)) as archive:
                for entry in archive:
                    require(entry.isdir() or entry.isfile())
                    require(not entry.name.startswith("/") and ".." not in entry.name.split("/"))
                    if entry.isfile():
    ...
    def retained_history(self):
        ...
        require([digest(canonical(e)) for e in entries] == self.admissions)
        nodes = sum(source_nodes(e) for e in entries)
    ...
    def evidence(self):
        ...
        require(e["provenance"] == self.saved("provenance"))
        wal = retained["session/egress-audit.wal"]
        require(wal.endswith(b"\n"))
        records = [json.loads(r, object_pairs_hook=unique_object) for r in wal.splitlines()]
        require(len(records) == e["audit"] == e["upstream"] == 1 and e["egress"]["records"] == records)
        r = records[0]
        ...
        require(e["upstreamRequests"] == [{"method": "GET", "path": "/credential", "credential": True}])
        completed = e["egress"]["completions"]
        require(len(completed) == 1 and e["egress"]["retired"] and not e["egress"]["uncorrelated"])
        c = completed[0]
        require((c["intentId"], c["sequence"], c["routeId"], c["responseCode"]) == (r["intent_id"], 0, r["route_id"], 200))
        require(c["completedAtMs"] >= r["timestamp_ms"] and c["durationMs"] >= 0)
        accounting = e["egress"]["accounting"]
        ...
        for export in e["exports"]:
            path, body = export["path"], export["body"]
            paths.append(path)
            require(path in ("/v1/traces", "/v1/metrics", "/v1/logs"))
            kind = {"/v1/traces": "Spans", "/v1/metrics": "Metrics", "/v1/logs": "Logs"}[path]
            require(set(body) == {"resource" + kind} and len(body["resource" + kind]) == 1)
            resource = body["resource" + kind][0]
            scopes = resource["scope" + kind]
            require(len(scopes) == 1)
            if kind == "Logs":
                ...
                for log in scopes[0]["logRecords"]:
                    log_count += 1
                    require(log["body"] == {"stringValue": "cogs.egress.complete"} and log["severityText"] == "INFO")
                    attrs = {a["key"]: next(iter(a["value"].values())) for a in log["attributes"]}
                    require(len(attrs) == len(log["attributes"]) == 11)
                ...
                items = scopes[0]["spans" if kind == "Spans" else "metrics"]
                require(items)
                for item in items:
                    ...
                    if kind == "Spans":
                        ...
                        require(re.fullmatch(r"[0-9a-f]{32}", item["traceId"]) and re.fullmatch(r"[0-9a-f]{16}", item["spanId"]))
                        telemetry_count += 1
                    else:
                        points = item.get("sum", item.get("gauge", {}))["dataPoints"]
                        require(points and all(int(p["asInt"]) >= 0 for p in points))
                        telemetry_count += len(points)
        require(log_count == 1 and paths.count("/v1/traces") == e["traces"] > 0 and paths.count("/v1/metrics") == e["metrics"] > 0)
        t = e["telemetry"]
        ...
        require(not git("fsck", "--full", "--no-reflogs", "--unreachable"))
        commit = git("rev-parse", "HEAD").decode().strip()
        require(all(m["commit"] == commit and "checkpoint_ref" not in m for m in maps))
        require(git("show", "HEAD:proof.txt") == b"alpha\n")
        ...
        require(not git("status", "--porcelain=v1", "--untracked-files=all"))
        bundle = self.receipt["user"]["bundle_digest"][7:]
        ...
        descriptor = e["exported"]["bundle"]
        require(e["exported"]["sensitive"] is True and descriptor["mode"] == "raw" and descriptor["file_count"] == 6)
        require(descriptor["bundle"] == "cogs-session-product-session")
        export_root = "session/sessions/product-session/exports/" + descriptor["bundle"] + "/"
        ...
        for file in manifest["files"]:
            data = retained[export_root + file["path"]]
            require(len(data) == file["bytes"] and digest(data)[7:] == file["sha256"])
        require(retained[export_root + "session.jsonl"] == files[native])
        exported_maps = json.loads(retained[export_root + "git-map.json"])["records"]
        require(exported_maps and maps[:len(exported_maps)] == exported_maps)
        require(json.loads(retained[export_root + "warnings.json"])["warnings"] == [])
    ...
    def dispatch(self, q):
        ...
        if op == "mkdir":
            ...
            require(re.fullmatch(r"/dev/loop[0-9]+", loop))
            self.record(name + "-loop", self.storage_observation(name)[0])
            self.record(name + "-mount-intent", True)
            self.command(["mount", "-o", "nodev,nosuid", loop, self.root + "/" + name])
            self.record(name + "-mount", self.storage_observation(name)[1])
            ...
            self.record(role + "-receipt", self.ids[role])
            created = self.inspect(cid)
            require(created["Image"] == spec["image"] and not created["State"]["Running"])
            self.ids[role]["environment"] = environment(created["Config"], role)
    ...
    def message(self, conn):
        ...
        if q["op"] in ("headers", "event", "turn", "persist", "history", "shutdown"):
            ...
            if q["op"] == "event":
                event = q["event"]
                require(set(event) == {"kind", "correlation_id", "request_id"})
            ...
            if q["op"] == "persist":
                ...
                self.nodes += nodes
            if q["op"] == "history":
                require(self.turns == 3 and [v["kind"] for v in self.publications].count("run_settled") == 3)
                self.retained_history()
            require(q["op"] != "shutdown" or self.released)
            self.shutdown = self.shutdown or q["op"] == "shutdown"
            reply = "granted"
        else:
    ...
    def settle(self):
        ...
        for role in reversed(list(self.ids)):
            held = self.ids[role]
            present = self.docker("ps", "-aq", "--no-trunc").decode().splitlines()
            ...
            if "pidfd" in held:
                ...
                os.close(held["pidfd"])
            del self.ids[role]
            self.record(role + "-retired", {"id": held["id"]})
        if self.disk is not None:
            require(self.docker("ps", "-aq", "--no-trunc") == b"")
        for role in ("trust", "sandbox", "worker"):
            names = os.listdir(self.control)
            require(role + "-intent" not in names or role + "-retired" in names)
        if not self.failed:
