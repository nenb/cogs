...
The image uses a digest-pinned Debian testing base and one dated testing snapshot without mixing stable packages into that base. Its OCI labels fix `dev.cogs.profile=insecure-container`, `dev.cogs.authority=functional-only`, and package policy `debian-testing-snapshot-20260829-insecure-conformance-v4`. Exact pins retain fixed util-linux `2.42.2-2`, Expat `2.8.3-1`, OpenSSL `3.6.3-1`, and OpenJDK 25 `25.0.4+7-1`. It deliberately retains OpenSSH plus curl, Git, pip/Python requests/httpx, npm/Node, Java, DNS tools, iproute2, iptables/nftables, netcat, and socat for compatibility and bypass probes. Those client/probe roots are forbidden from the production sandbox package policy.
