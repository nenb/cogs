...
The container has a read-only root filesystem, bounded tmpfs mounts, a temporary workspace volume, and no privileged/host mounts. A fixed `host.docker.internal` gateway alias lets candidate conformance address the trusted host proxy without exposing an admin endpoint. The profile intentionally retains ordinary Docker bridge networking: guest root can bypass proxy variables, so it cannot support a network-isolation claim.
...
The image uses a digest-pinned Debian base and dated Debian snapshots. Its OCI labels fix `dev.cogs.profile=insecure-container`, `dev.cogs.authority=functional-only`, and package policy `debian-trixie-snapshots-20260713-20260815-insecure-conformance-v3`. The security snapshot includes the util-linux `2.41.5-0+deb13u1` family fix for CVE-2026-53615; the exact `openjdk-21-jre-headless=21.0.12+8-1~deb13u1` pin includes the Debian fixes for CVE-2026-41254 and CVE-2026-47063. It deliberately retains OpenSSH plus curl, Git, pip/Python requests/httpx, npm/Node, Java, DNS tools, iproute2, iptables/nftables, netcat, and socat for compatibility and bypass probes. Those client/probe roots are forbidden from the production sandbox package policy.

Every external lifecycle operation and CI phase has a hard deadline. Lifecycle commands are serialized per state directory, require a matching ownership sentinel before deleting state, clean partial resources on failure or interruption, and emit `insecure-container` / `functional-only` results. State directories are restricted to one direct child of `.cogs-dev`; proxy endpoints must be non-credentialed HTTP(S) host-and-port URLs. The label-gated CI workflow exercises create, verify, reset, post-reset verify, and destroy, then publishes only schema-valid applicability-aware functional evidence. Once the first proxy adapter exists, that workflow is extended to run the `insecure-container` conformance profile; smoke evidence alone never selects a proxy or satisfies Stage 1 security acceptance.
