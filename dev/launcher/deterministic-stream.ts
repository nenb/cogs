...
function s309BashTool(port: number | undefined) {
  ...
  return Object.freeze({
    ...
    name: "bash",
    arguments: Object.freeze({
      command: `set -eu\ntest "$(cat /workspace/s3-09/proof.txt)" = "alpha\nbeta"\ncurl_path=$(command -v curl)\ntest "$curl_path" = /usr/bin/curl\ntest -f "$curl_path"\ntest ! -L "$curl_path"\ntest -x "$curl_path"\ntest "$(stat -c '%u:%g:%a' "$curl_path")" = 0:0:755\ncurl_verify=$(/usr/bin/dpkg --verify curl)\ntest -z "$curl_verify"\nallowed=$(${s309Curl(port, "credential")})\ntest "$allowed" = '200 1 192.0.2.1 18080 0'\ndenied=$(${s309Curl(port, "allowed")})\ntest "$denied" = '403 1 192.0.2.1 18080 0'\ncd /workspace\ngit add s3-09/proof.txt\ngit commit -q -m s3-09-integrated\ni=0; while [ "$i" -lt ${LAUNCHER_DETERMINISTIC_S309_UPDATE_COUNT} ]; do printf '${LAUNCHER_DETERMINISTIC_S309_UPDATE_LINE}'; sleep ${LAUNCHER_DETERMINISTIC_S309_UPDATE_DELAY}; i=$((i+1)); done\nprintf '${LAUNCHER_DETERMINISTIC_S309_BASH_MARKER}'`,
    }),
  });
}

function s309Curl(port: number, path: "credential" | "allowed"): string {
  return `/usr/bin/env -i PATH=/usr/bin:/bin HOME=/nonexistent /usr/bin/curl -q --config /run/cogs-launcher-egress/curl-proxy.conf --proxy-basic --proxy http://192.0.2.1:18080 --noproxy '' --cacert /run/cogs-launcher-egress/proxy-ca.pem --http1.1 --proto '=https' --connect-timeout 3 --max-time 10 --silent -o /dev/null -w '%{http_code} %{proxy_used} %{remote_ip} %{remote_port} %{ssl_verify_result}' https://localhost:${port}/${path} 2>/dev/null`;
}
