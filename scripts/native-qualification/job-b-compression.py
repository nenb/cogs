#!/usr/bin/python3
"""Job B client for the common-owned compression qualification."""
...
OPERATION = "B"
FAILURE_PHASE = "compression"
DIAGNOSTIC_LIMIT = 2_048


class QualificationError(RuntimeError):
    """The fixed Job B workflow entry was not selected."""
...
def _combine(primary: BaseException | None, cleanup: BaseException) -> BaseException:
    if primary is None:
        return cleanup
    return BaseExceptionGroup("Job B operation and settlement", [primary, cleanup])
...
def _dispatch(
    ...
    _require(__debug__ and arguments == ["--workflow-bound"], "fixed Job B workflow entry")
