#!/usr/bin/python3
"""Thin client for the common-owned ordinary runtime integration."""
...
class QualificationError(RuntimeError):
    """The fixed integration workflow entry was not selected."""
...
def _combine(primary: BaseException | None, cleanup: BaseException) -> BaseException:
    if primary is None:
        return cleanup
    return BaseExceptionGroup("integration operation and settlement", [primary, cleanup])
...
def _dispatch(
    ...
    _require(entry_is_fixed and os.geteuid() != 0, "fixed unprivileged integration workflow entry")
