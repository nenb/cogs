#!/usr/bin/python3
"""Job A client for the common-owned runtime-mapping qualification."""
...
OPERATION = "A"
FAILURE_PHASE = "runtime_mappings"
DIAGNOSTIC_LIMIT = 2_048


class QualificationError(RuntimeError):
    """The fixed Job A workflow entry was not selected."""
...
def _combine(primary: BaseException | None, cleanup: BaseException) -> BaseException:
    if primary is None:
        return cleanup
    return BaseExceptionGroup("Job A operation and settlement", [primary, cleanup])
...
def _dispatch(
    ...
    _require(__debug__ and arguments == ["--workflow-bound"], "fixed Job A workflow entry")
