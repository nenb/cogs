...
PRODUCT_TEST_REMAINING_LINES, PRODUCT_TEST_REMAINING_BYTES = 19527, 19000000
PRODUCT_TEST_FORECASTS = {'route': 0, 'revocation': 0, 'relay': 1400, 'lifecycle': 1000, 'completion': 0, 'integration': 17127}
...
PRODUCT_TEST_GLOBAL_LINE_FORECAST, PRODUCT_TEST_GLOBAL_BYTE_FORECAST = 37527, 27000000
...
PRODUCT_TEST_TASK_SPECS = (('pre_h_final_governance', 2200, 1000000, {'integration': (2200, 1000000)}), ('aws_principal_denial_and_cycle_custody', 7000, 8000000, {'integration': (7000, 8000000)}), ('protected_product_and_kvm_contract', 4000, 3200000, {'integration': (1600, 1300000), 'lifecycle': (1000, 1000000), 'relay': (1400, 900000)}), ('generated_readiness_and_no_mint_authorization', 1800, 2000000, {'integration': (1800, 2000000)}), ('final_hgq_control_reserve', 4527, 4800000, {'integration': (4527, 4800000)}))
PRODUCT_TEST_PRE_H_TASK_MAXIMA = {'pre_h_final_governance': (2200, 1000000), 'aws_principal_denial_and_cycle_custody': (7000, 8000000), 'protected_product_and_kvm_contract': (4000, 3200000), 'generated_readiness_and_no_mint_authorization': (1800, 2000000), 'final_hgq_control_reserve': (1027, 1300000)}
...
def _remediation_budget():
    ...
    _require(data["version"] == "cogs.external-review-remediation-budget/v1"
             ...
             and data["global_gross_line_high"] == 67_527 and type(data["global_gross_byte_high"]) is int and data["global_gross_byte_high"] == REMEDIATION_GLOBAL_BYTE_HIGH)
...
def _product_test_budget(data, allocations):
    ...
    for task, expected_task in zip(tasks, PRODUCT_TEST_TASK_SPECS):
        ...
        if name == "aws_principal_denial_and_cycle_custody":
            expected_keys |= {"forecast_slices", "withheld_forecast", "ingress_inventory", "decode_only_non_ingress", "non_aws_diagnostic_routes", "budget_consumers"}
