...
def _product_test_budget(data, allocations):
    ...
    for task, expected_task in zip(tasks, PRODUCT_TEST_TASK_SPECS):
        ...
        if name == "aws_principal_denial_and_cycle_custody":
            _require(task["measured_slices"] == [
                {"name": "deployment_and_direct_python_ingress", "gross_lines": 5600, "gross_bytes": 6300000},
                {"name": "workflow_and_opentofu_compatibility", "gross_lines": 1400, "gross_bytes": 1700000},
            ] and sum(item["gross_lines"] for item in task["measured_slices"]) == lines
                     and sum(item["gross_bytes"] for item in task["measured_slices"]) == raw_bytes)
        if name == "final_hgq_control_reserve":
