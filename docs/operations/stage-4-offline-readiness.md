...
[`stage4-runtime-artifact-closure.ts`](../../scripts/stage4-runtime-artifact-closure.ts) builds and purely classifies the strict v3 authenticated-runtime evidence. It performs no filesystem, process, network, provider, Kubernetes, Docker, or external-model operation. The evidence binds:
...
Readiness v4 preserves the former protected-main assertion SHA-256 `ad45d6b0a114c481eb869daff38f4ddc669801e8a5b5d808c404b506ae33c450`, independent review SHA-256 `f6a607349062f5ca9211978600012b4cd16651af984c083a0329646a74da6a3a`, workflow run `30852317459`, image-source revision `d3ddb987ceeec0bae0fa2d89fdc134187a0d1de3`, and exact GHCR digests as historical records only. The Pi 0.84.2 source and worker dependency closure postdate those images, so they are incompatible with the current source and cannot remain the active candidate. `release_image_set_present=false`, static image identity closure is false, and `RELEASE_IMAGE_SET_ABSENT` is restored until a new protected-main worker/sandbox set is published and independently reviewed.

This static binding does not establish runtime image closure. Envoy remains content-pinned without a publisher signature. The historical OpenBao v2.6.1 identity remains signed and byte-bound, but its fixed HIGH findings retire it from current selection; `OPENBAO_FIXED_RELEASE_IMAGE_ABSENT` blocks readiness and no ignore is active. None of the remaining selected images or runtime artifacts has been observed together in EKS/Kata. `exact_image_runtime_closure_satisfied=false` remains. Active NIC v2 resolves source capability only; launch-template contents and provider truth remain unobserved. The readiness blockers retain issue #42, fixed OpenBao release image, current-source release image set, AWS image/kernel, account, current price, current quota, separated identities, campaign approval/envelope, and executable provider route. All cloud, provider, Kubernetes, current-resource, zero-resource, Stage 4 exit, production, and release claims remain false.

Regenerate the authenticated evidence alone with `npx tsx scripts/stage4-runtime-artifact-closure-regenerate.ts`. The full `npm run readiness:regenerate` procedure regenerates the runbook index, schema inventory, the runtime evidence's deterministic schema-inventory anchor, authenticated evidence, source/local-validation inventories, classifier anchors, and the v4 package. If integration changes a schema or any frozen source byte, rerun the full procedure rather than hand-editing digests.
