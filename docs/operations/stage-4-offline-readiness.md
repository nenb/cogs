...
This static binding does not establish runtime image closure. Envoy remains content-pinned without a publisher signature; OpenBao is signed; none of the selected images or runtime artifacts has been observed together in EKS/Kata. `exact_image_runtime_closure_satisfied=false` remains. Active NIC v2 resolves source capability only; launch-template contents and provider truth remain unobserved. The readiness blockers retain issue #42, AWS image/kernel, account, current price, current quota, separated identities, campaign approval/envelope, and executable provider route. All cloud, provider, Kubernetes, current-resource, zero-resource, Stage 4 exit, production, and release claims remain false.
