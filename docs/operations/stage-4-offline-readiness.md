...
Regenerate the authenticated evidence alone with `npx tsx scripts/stage4-runtime-artifact-closure-regenerate.ts`. The full `npm run readiness:regenerate` procedure regenerates schema inventory first, then this evidence, source/local-validation inventories, classifier anchors, and the v2 package. If integration changes a schema or any frozen source byte, rerun the full procedure rather than hand-editing digests.
