...
This classifier has no override or recovery operation.

## One session object graph

Exactly one trusted resource contains exactly `worker` and `proxy`. Exactly one separate Kata sandbox resource contains exactly `sandbox`, names `kata-qemu-cogs`, has no trusted sidecar, mounts only the workspace, and cannot see trusted session state. Both resources have the same session ID and bind to one immutable launch-document digest. The trusted resource mounts only trusted session state and never the untrusted workspace.
...
Canonical JSON input is limited to 128 KiB, 512 snapshotted nodes, depth 16, 2048 UTF-8 bytes per string, and one resource per role. Unknown fields, accessors, sparse/extended arrays, symbol keys, non-plain prototypes, proxy traps, unsafe numbers, malformed UTF-8/JSON, noncanonical bytes, and bound violations fail closed. Getter bodies are never invoked. A bound violation preserves uncertainty rather than authorizing cleanup or retry.
