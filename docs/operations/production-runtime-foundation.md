...
Custody is an independent root process outside the candidate cgroup. It retains exclusive intents and exact container receipts; a lost create/start response never authorizes name-based discovery or deletion. Helpers stop before exec and enter the subordinate cgroup. A helper's complete service bound is 20 seconds: 15 seconds of command observation plus five seconds for its two explicit two-second retirement waits and local overhead. Initial gate acquisition has a 65-second bound beyond its three-helper high; the later snapshot-control acquisition has a separate 25-second bound beyond its one-helper high, while runtime skill-lease ping/release retains its two-second deadline. The complete receipt, publication source identities, and both containers are fully authenticated once; later control admissions require the exact canonical receipt, publication source inodes, and retained exact worker/sandbox pidfds alive without helper-backed Docker inspection on the worker event loop. Periodic synchronous gate heartbeat begins only after production startup and asynchronous snapshot-control acquisition settle, and its cached-live reply remains bounded to five seconds. Owner peer-idle observation is 65 seconds. The initial lease-receipt publication wait is 105 seconds, beyond the residual worker-start helper plus four worker/sandbox authentications; parent worker-status sampling uses the retained pidfd at 500-millisecond intervals and invokes at most three exact-ID Docker state observations only after observed process exit so the daemon can settle its exit state. An authenticated stopped worker fails at `status` with only `application-exit`, `skill-gate-exit`, or `other-exit`, never a raw exit value or process output. Both containers have read-only rootfs, no log driver, no privilege, default seccomp, no-new-privileges, exact private bind mounts, no source/containing-directory aliases, and memory/swap/CPU/PID limits. PID/pidfd, image, mount namespace, live RO flags, device/inode, capabilities and cgroup limits are checked before releasing the inert worker. Unix kernel-peer authentication, nonce/sequence checks and heartbeats bind the gate and one-time snapshot consumer lease. Control loss/deadline fails closed; it never proves retirement.
