...
The real `ssh2` adapter authenticates the pinned connection. A successful SFTP `OPEN` callback with a missing, oversized, or hostile handle is terminal uncertainty: the server may already have created a file or allocated a remote handle, so Cogs neither closes the malformed value nor speculatively unlinks by pathname, and it retains channel/permit ownership while sealing readiness. A duplicate later callback cannot heal that first observation. The manager exposes bounded permit/semaphore leases and manager-owned SFTP/exec channel access; raw `ssh2` clients/channels are not exposed to Pi-facing code. Every real SFTP or bash exec channel is opened only after a held permit, so the configured maximum bounds actual concurrent channels. The architecture retains ADR 0013's no-guest-daemon modular trust-boundary adapter path.
...
- `write` returns `{ ok, path, bytesWritten, atomic: true, fsync: "openssh" }`. Content is bounded UTF-8. Final symlinks and non-regular targets are rejected. Existing-file permissions come from `fstat` on the retained opened original handle; new files use `0600`. Missing, malformed, non-regular, set-ID, and sticky mode metadata fails closed before masking. The adapter opens a cryptographically unpredictable exclusive `0600` temporary sibling, verifies that same handle is an empty regular `0600` file before writing, uploads bounded chunks, applies and verifies ordinary mode through handle-relative `fchmod`, then requires OpenSSH `fsync`, close, and `posix-rename`. Confirmed prepublication failures and rejected renames clean only the temp. Ambiguous callbacks retain channel ownership, fail SSH closed, and do not race speculative cleanup or claim rollback.
- `edit` performs a bounded strict-UTF-8 read, derives mode from that same opened read handle, requires nonempty `oldText`, requires exactly one occurrence, applies exact replacement only, then uses the same atomic write path. Missing or duplicate matches leave the target unchanged.

SFTP v3 cannot eliminate all guest-side TOCTOU races against a malicious root guest. Owner/group, ACLs, xattrs, hard links, and concurrent writers are outside this permission-preservation contract. These checks are file-tool correctness and containment controls for trusted launch roots; they are not a host-security boundary. The `bash` adapter makes guest files accessible to the guest process, so these file tools make no false isolation or host-secret claim.
