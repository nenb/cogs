...
The generation-aware polling watcher covers credentialed integration routes, not `launch.model.credential_handle`. A model API key is resolved into trusted worker memory for that worker generation. Deleting, rotating, or recreating its OpenBao record does not by itself invalidate that hydrated key or cancel model work. Current containment requires the external lifecycle owner to close and verifiably retire the affected worker before replacement; provider-side key revocation remains independently required where available. Do not describe OpenBao model-key deletion as bounded worker revocation.
