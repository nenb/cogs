...
Adding the `security` label triggers the KVM qualification workflow. The same labelled path runs the `insecure-container` SSH/SFTP smoke and the selected Envoy proxy conformance runs; these reports are functional-only and stub-aware until Stage 3 replaces fixture dependencies. The same job then creates the isolated Debian guest and runs the full shared matrix against Envoy as authoritative-local evidence. Historical mitmproxy comparison and OpenBao fixture code remain in the repository, but their expiring candidate-only vulnerability exceptions were removed instead of renewed, so neither is run as an active Stage 3 path or release fallback. OpenBao-dependent model-auth, real-runtime, and launcher smoke remain disabled until one clean authenticated replacement image restores all three together. Any qualification, setup, suite, evidence, or teardown failure fails the job without fallback. Routine CI artifacts need not be committed; release-candidate reports belong under `docs/security-evidence/`.
