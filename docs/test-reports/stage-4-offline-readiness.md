...
The two runtime-artifact identity blockers are therefore removed. Active personal-fork NIC v2 also resolves source-level external launch-template selection, so the historical NIC capability blocker is removed. Protected-main run `30852317459` produced a canonical assertion and independently reviewed exact worker/sandbox digests. Readiness v4 preserves those records as historical because its worker predates the Pi 0.84.2 source closure; `RELEASE_IMAGE_SET_ABSENT` is therefore restored until a new protected-main publication and review. This does **not** establish runtime observation closure: exact Envoy lacks a publisher signature; launch-template contents, the EKS AMI ID/running kernel, the selected containerd override, and the composed images remain AWS/EKS/Kata-unobserved. AWS image/kernel, account, price, quota, identity, approval, and provider-route blockers remain.
