...
1. `createAgentSession()` runs headlessly with an explicitly constructed `ModelRuntime`, application-owned in-memory credential storage, in-memory settings, a persistent `SessionManager`, and a closed custom `ResourceLoader`.
...
3. A non-secret runtime API key supplied through `ModelRuntime.setRuntimeApiKey()` is resolved for each fake stream call, creates no `auth.json`, and is absent from JSONL.
...
- `ModelRuntime.create()`, `setRuntimeApiKey()`, `getAuth()`, `getProviderAuthStatus()`, and `removeRuntimeApiKey()`
...
- the public `session.agent.streamFunction` property for deterministic Stage 0 fake streaming only
