...
This map supersedes the readiness conclusion, but not the historical facts or no-AWS boundary, of ADR 0266. As adjudicated by the accepted, unmerged ADR 0351 correction, its historical non-AWS gates remain facts while the corrected source must pass protected validation before a **new** implementation H is selected. No old H, G, or Q may be mutated or retroactively blessed. A passing local or diagnostic result cannot authorize AWS activity or close Issue #42.
...
After the corrected package is frozen as a new H, stop. A new G must control that exact H and a new Q must qualify that exact G. Separate authorization is required first for bounded read-only AWS discovery/planning and later for exactly one attempt-one, two-job seven-cycle execution batch under one signed v6 approval. Each executor and observer assumption derives its duration independently immediately before the bounded configure action, retains at least 900 seconds of expiry margin, and must complete that action within 10 minutes. Before any future authorized run, both role definitions must have `MaxSessionDuration >= 23400`; reducing that prerequisite cannot be compensated by workflow timeouts. The diagnostic route emits only a non-authoritative diagnostic receipt and cannot satisfy any acceptance row. No credential use, provider/OpenTofu effect, SSM command, deployment, inventory query, campaign, or AWS cleanup is authorized by this map.
