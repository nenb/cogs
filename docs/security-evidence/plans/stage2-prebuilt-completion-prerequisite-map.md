...
After the corrected package is frozen as a new H, stop. A new G must control that exact H and a new Q must qualify that exact G. Separate authorization is required first for bounded read-only AWS discovery/planning and later for exactly one attempt-one, two-job seven-cycle execution batch under one signed v6 approval. The diagnostic route emits only a non-authoritative diagnostic receipt and cannot satisfy any acceptance row. No credential use, provider/OpenTofu effect, SSM command, deployment, inventory query, campaign, or AWS cleanup is authorized by this map.
