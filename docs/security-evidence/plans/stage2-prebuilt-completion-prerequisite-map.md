...
ADR 0368 accepts the replacement qualification and authorizes only a protected planning-control successor R before credentials. R must correct the dormant workflow's V5 labels and unauthenticated V3 envelope path to the accepted V6 package and V4 envelope, pass protected checks, and receive independent acceptance. Only then may bounded IAM/OIDC bootstrap and a zero-resource baseline precede exactly one first-created attempt-one read-only planning run bound to H, G, Q, qualification `36036544983`, artifact `10829051963`, and its archive digest.

Independent acceptance of all seven plans is required before one plan-bound signed V6 approval and exactly one attempt-one, two-job seven-cycle execution batch with at least 32,400 actual seconds remaining before production credentials. Each executor and observer assumption uses one bounded root-staged OIDC/STS helper that samples every applicable deadline immediately before its single STS request, retains at least 900 seconds of expiry margin, and rejects a returned expiration outside the campaign runway; the entire helper must complete within 10 minutes. Before the authorized run, both role definitions must have `MaxSessionDuration >= 30600`; reducing that prerequisite cannot be compensated by workflow timeouts. The diagnostic route emits only a non-authoritative diagnostic receipt and cannot satisfy any acceptance row. Failure, retry, mismatch, uncertain cleanup, or unexpected inventory is terminal and permits cleanup only; Stage 4 remains separate and non-authorized.
