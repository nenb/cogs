...
`schemas/stage5-privacy-deletion-suite-v1.json` and `schemas/stage5-privacy-deletion-report-v1.json` define the disjoint issue #365 local/static synthetic privacy domain. The pure scanner covers metadata-only OTLP/log/report/event/crash fixtures and the authenticated sensitive raw-export descriptor boundary, while runtime-only generated canaries cover prohibited content categories. The committed fixture contains no canary or raw/sensitive payload. Findings are bounded categories/counts/digests only. Recursive Proxies are rejected before reflection, getters are not invoked, and input is bounded before hashing.
