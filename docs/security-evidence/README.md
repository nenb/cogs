...
`schemas/stage4-offline-readiness-package-v1.json` and `schemas/stage4-offline-readiness-verdict-v1.json` define the issue #357 local/static readiness domain. A bounded trusted local preparation executable separately authenticates the complete chart inventory, exact values, and pinned Helm executable identity/version, runs client-only local `helm template` twice, compares fresh output with committed bytes, and emits a canonical digest-only receipt. The pure classifier executes nothing and binds that receipt plus complete Stage 4 source, chart, values, renders, image record, NIC, runtime record, Stage 4/5 schemas, local validation, and a semantic root. `local_preparation_complete=true` is fixed to `bounded-package-assembly-and-local-validation-only`; exact image/runtime closure remains false because release images are absent placeholders and containerd/QEMU artifact identities plus the EKS image/kernel are unresolved. Campaign request readiness, approval, cloud authority/execution, provider truth, current/zero resources, Stage 4 exit, and release eligibility remain false. The NIC v1 blocker remains historical; active v2 source capability does not close launch-template observation, node-image, runtime, campaign, or release blockers. Account and separated identities are absent, price/quota are undiscovered, and every exact resource count, spend/TTL, destroy path, and service-specific non-tag-only inventory scope is proposal-only. See [`docs/operations/stage-4-offline-readiness.md`](../operations/stage-4-offline-readiness.md) and [`docs/test-reports/stage-4-offline-readiness.md`](../test-reports/stage-4-offline-readiness.md).
