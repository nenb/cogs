...
`schemas/stage4-offline-readiness-package-v1.json` and `schemas/stage4-offline-readiness-verdict-v1.json` define the issue #357 local/static readiness domain. The canonical package binds exact source/chart/values/byte-identical-render/image/NIC/runtime/schema/local-validation bytes plus a domain-separated semantic root over its fixed proposal, blocker, identity, stop/destroy, one-attempt, and revalidation shape. A valid verdict may set only `local_preparation_complete=true`; campaign request readiness, campaign approval, cloud authority/execution, provider truth, current/zero resources, Stage 4 exit, and release eligibility remain false. The current NIC `v0.11.0` / module `0.7.0` capability blocker and unresolved EKS AMI/image/kernel are sticky. Account and separated identities are absent, price/quota are undiscovered, and the bounded region/instance/node/spend/TTL/resource graph is proposal-only. The pure classifier has no executable provider route and returns only categorical uncertainty. See [`docs/operations/stage-4-offline-readiness.md`](../operations/stage-4-offline-readiness.md) and [`docs/test-reports/stage-4-offline-readiness.md`](../test-reports/stage-4-offline-readiness.md).
