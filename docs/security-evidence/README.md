...
Historical v1 and active v2 readiness schemas define issue #357's local/static domain. A bounded trusted local preparation executable authenticates the chart inventory, exact values, and pinned Helm executable identity/version, verifies client-only zero manifests and repeated NOTES output, and emits a canonical digest-only receipt. The pure v2 classifier executes nothing and binds that receipt plus complete Stage 4 source, chart, values, renders, placeholder image record, active NIC v2, runtime pins, authenticated-runtime evidence, Stage 4/5 schemas, local validation, and a semantic root. Containerd `2.2.1` and active Kata-bundled QEMU `11.0.1` candidate artifact identities are authenticated; QEMU `8.2.2` is historical only. Exact image/runtime observation closure remains false because release images are `.invalid` placeholders and the region-specific EKS AMI/running kernel and selected runtime are unobserved. Active readiness removes only the historical NIC capability and containerd/QEMU artifact-identity blockers. Issue #42, AWS image/kernel, account, current price/quota, separated identities, approval/envelope, release images, and provider route remain blocking. Campaign authority/execution, provider truth, current/zero resources, Stage 4 exit, and release eligibility remain false. See [`docs/operations/stage-4-offline-readiness.md`](../operations/stage-4-offline-readiness.md) and [`docs/test-reports/stage-4-offline-readiness.md`](../test-reports/stage-4-offline-readiness.md).
