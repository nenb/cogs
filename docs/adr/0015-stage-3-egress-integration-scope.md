...
- The repository must vendor only the exact minimal extracted 25 `.proto` source files for the Envoy 1.38.3 plus BCR-pinned Google, XDS, UDPA, validate, and protobuf WKT closure under a trusted non-`src` audit/regeneration path. It must not vendor whole upstream source archives for this purpose.
- Generate one `FileDescriptorSet` artifact for `envoy/service/auth/v3/external_auth.proto` from those checked source files. Generation must use `--include_imports` and must not use source info.
...
- Keep a manifest containing exact per-file source origins, licenses, source file hashes, descriptor size/hash/file list, `protoc` URL/version/hash, and loader options.
...
CI must download pinned Linux x86_64 `protoc 33.1`, verify its hash and version, regenerate the descriptor byte-for-byte from the checked minimal 25-file source closure, decode/assert the exact 25-file descriptor closure, assert the exact `/envoy.service.auth.v3.Authorization/Check` unary service, and run a Node loader smoke with the manifest loader options. Production/runtime images must exclude the source proto tree and `protoc`; they must contain only the required descriptor artifact, manifest, and license notices. Runtime never reads source protos.
