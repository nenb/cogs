# ADR 0091: Require exact production ownership before native qualification
...
- **Current disposition:** **Correction authorized; native execution remains blocked**

The five final hostile reviews blocked native execution at `ea6e74f`. This decision authorizes only the closed source correction and portable/static verification below. It does not authorize a native selector, workflow dispatch, sudo/native primitive execution, provider, cloud, or AWS operation.

## 1. Controlling outcome and boundaries

This ADR authorizes only the closed source correction and portable/static verification described here. It must retain these boundaries:
...
4. **AWS remains a separate mandatory boundary.** Neither this decision, ADR 0091, a portable pass, clean reviews, nor a later native result grants AWS, provider, OpenTofu, deployment, campaign, production, release, or issue-closure authority. Any AWS action requires its own later accepted decision under the existing AWS controls.
...
The reports agree on the blockers but sometimes prescribe apparently different API or ownership shapes. This ADR resolves them as follows.
...
No final report identified a P0 or standalone P3. Fresh reviewers may do so; This ADR requires their resolution rather than treating this table as exhaustive authority.
...
Permitted only under this accepted correction ADR:
...
| Exact surface | Reviewed gross | ADR 0090 high | ADR 0091 high | Justification |
| --- | ---: | ---: | ---: | --- |
...
| Exact file | Reviewed gross | ADR 0090 high | ADR 0091 high | Justification |
| --- | ---: | ---: | ---: | --- |
...
- exact gross additions remain within every accepted high and subtotal;
