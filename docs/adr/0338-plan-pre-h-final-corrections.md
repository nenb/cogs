...
At 118,000 microUSD/hour, `ceil(302 × 118,000 / 60) = **593,934 microUSD**` is
...
593,934, with **no normal authority**. It requires a separately approved incident budget before any continued
...
### Latest P1 review corrections (authoritative and plan-only)

This subsection supersedes the P1 campaign-model statements above where they
conflict. It changes no source, trust policy, role, backend, budget, workflow,
or cloud state; every existing pre-credential and effect denial remains literal.

**Identity and issuance.** `SourceIdentity` is one chain-safe invariant value,
not a purpose carrier: `cogs-${campaignDigest}-${cycle}-${epoch}-${authorityId}`.
It is ASCII in the STS allowed set, at most 64 characters, is CAS-bound before
the first assumption, and is identical on every role hop for that authority.
Trust requires that exact value on every hop; replacement, omission, mutation,
or a different value after role chaining denies. Purpose instead is the bounded
session name, required transitive `Purpose` session tag, and the external
custody intent. The intent binds the issuer, target role, purpose, request ID,
request-start, maximum duration, tags, external ID where applicable, and
expected generation. Session names and transitive tags are exact, bounded,
immutable, and rechecked at each hop; they cannot substitute for identity.

Every normal or backend issuance class is separately named and server-fenced:
normal-cycle and backend `planner`, `apply`, `show`, and `destroy`. Before a
request, custody CAS-persists and reads back its immutable issuance record and
issuance intent, including request-start and an absolute `issuance_cutoff`.
Each target trust policy enforces the exact identity/tags and
`DateLessThan: {"aws:CurrentTime": issuance_cutoff}`. The issuer refuses to
start a process unless `issuance_cutoff - request-start >= 900 seconds +
recorded skew`; the normal maximum is 1,800 seconds and each backend maximum is
900 seconds. A lost, duplicate, or unknown response is never retried and has
conservative expiry exactly `issuance_cutoff + maximum-duration + recorded
skew`, followed by the existing fence wait.

The custody broker, not a normal parent credential, independently assumes the
one exact backend role for each persisted OpenTofu process intent. Thus each
planner/apply/show/destroy backend session has its own cutoff-bound issuance,
same invariant `SourceIdentity`, bounded purpose/session tag, and custody
record; it is not claimed to end at a contradictory parent-session expiry.
The broker remains unable to perform workload, backend-data, cleanup, or
custody-store actions. No process starts on a merely client-clock cutoff.

**Pre-provisioned cleanup trust.** Before the campaign, a distinct
control-plane principal pre-provisions two disabled cleanup executor trust
policies (one per possible epoch) and their roles. Neither cleanup issuer nor
cleanup executor can create, edit, activate, or relax either policy. When an
epoch is needed, that control-plane principal CAS-activates the pre-provisioned
policy against the immutable issuance record, atomically binding its exact
`DateLessThan` cutoff, role, invariant identity, external ID, GitHub claims,
transitive tags, authority ID, and policy version. Custody version-reads the
activation before OIDC is requested. Epoch two's policy is already
pre-provisioned before the campaign; it is activated only after epoch one's
recorded/conservative expiry plus 120 seconds. A failed CAS, readback, or
activation is sticky uncertainty; cleanup never self-amends trust.

**Operation closure.** The generated permission closure is pinned to a
versioned provider-operation inventory and `INVENTORY_QUERIES`; it must first
fail closed on any inventory/version/hash mismatch, then compare every
operation to the generated per-principal IAM policy. A later authenticated
real-control-account compatibility gate repeats that inventory-to-policy check
against the pinned provider before any credential or workload effect. The
normal, cleanup, observer, and fence lists therefore explicitly include IAM
`TagRole`, `UntagRole`, `TagInstanceProfile`, `UntagInstanceProfile`, EC2
`DescribeVpcAttribute`, and `DescribeLaunchTemplateVersions`, in addition to
all provider-required reads. Cleanup enumerates every dependency operation:
SSM cancel/list/get; instance termination; volume and ENI detach/delete; EIP
release; launch-template delete; security-group egress revoke/delete; route
and association delete/disassociate; gateway detach/delete; subnet/VPC delete;
scheduler and budget delete; and IAM policy detach/delete, instance-profile
role removal/delete, role-policy delete, role deletion, and tag removal. No
unlisted operation, wildcard mutation, or cleanup create is implied.

The three AWS forecast categories are **enforced**, not advisory. The checked
allocation assigns every AWS-task path to exactly one immutable category
(deployment/custody/direct ingress, pinned OpenTofu/backend compatibility, or
approval/staging/focused authority tests), measures gross no-deletion
consumption per category, and rejects an exact-category overrun as well as the
task/holdback overrun. The no-mint closure also names every downstream grant
consumer: the rehearsal workflow, grant issuer and both grant tests, full and
readiness rehearsal cycle consumers, and both rehearsal shell entries. Each
remains candidate/tree/package/expiry bound and denied before cloud credential
or AWS effect.

**Arm before launch.** Before the first billable `RunInstances` request, the
future control plane must durably arm and read back an account-level campaign
budget, independent alarm, and termination path. They use the approved account
and campaign limit, are outside executor success reporting and cleanup logic,
and remain effective if the launcher crashes, loses its launch response, or
never records an instance ID. `RunInstances` is denied unless all three
prelaunch receipts and their exact generation are present. The required fake
and later authenticated compatibility tests include a lost launch response and
prove that the independently armed alarm/terminator still fences the resulting
or ambiguous resource; an alarm, terminator, or exhausted budget is not a
success or settlement claim.

### CFF P1 and mechanical P2 correction (authoritative and plan-only)

This subsection supersedes any conflicting earlier ADR 0338 language. It is a
future design/test contract only: it does not alter a role, IAM policy, trust
policy, backend, source-inventory limit, OpenTofu installation, credential,
...
then disables that role's assume trust, attaches the named explicit deny-all,
and observes the exact IAM readback for both mutations. No role is removed from
the fence until terminal settlement.

After propagation, cleanup may begin only after the maximum over all normal and
backend issuance classes of `recorded NotAfter + recorded skew` or, for a lost/
unknown issuance, `issuance_cutoff + maximum-duration + recorded skew`, plus the
fixed 120-second fence skew. This is a maximum, never a per-role shortcut; a
short normal session cannot shorten a later backend session. Bounded probes with
the recorded old sessions must observe denial for one formerly allowed workload
operation and one backend state/lock write. They diagnose failure but do not
shorten that maximum wait. The fake recovery suite must leave a **paused backend**
state/lock write after admission, lose its updater/lease, perform the complete
role fence and maximum wait, and prove that resumed write is denied with no
second write or cleanup admission before the wait.

**Cleanup activation is a cross-service state machine.** DynamoDB CAS is used
only to persist the immutable epoch issuance record and to elect one bounded
trust updater (`prepared → updater-owned → IAM-update-observed → propagated →
activation-receipt`). It is not, and must never be described as, an atomic CAS
on IAM. The elected control-plane updater submits the exact pre-provisioned
trust document, then reads IAM until the exact role and canonical policy hash
are observed; its DynamoDB generation records that observation, not a fictional
cross-service transaction. It waits bounded IAM propagation and performs
bounded STS probes: the disabled/pre-activation assumption must deny and the
single exact post-activation assumption may accept only with the persisted
identity, external ID, tags, GitHub claims, epoch, and `DateLessThan` cutoff.
The successful observation, policy hash/version, probe identities/outcomes, and
generations form the durable activation receipt before any cleanup action.

If the updater crashes, loses an IAM response, or its DDB lease expires, a
successor CAS-elects only after reading both DDB and IAM. It may advance when
it observes the exact intended document and records that fact; conflicting,
missing, or ambiguous IAM state is sticky uncertainty, not an overwrite or
blind retry. A stale updater cannot commit after losing the DDB generation.
The cleanup issuer/executor never updates trust. The pre-provisioned trust has
its exact server-enforced `DateLessThan: {"aws:CurrentTime": issuance_cutoff}`;
a paused or late OIDC/STS issuance after that cutoff denies even if an old
activation receipt exists.

**Permission direction is fixed.** The observer is strictly read-only: it has
only `GetCallerIdentity` and the enumerated describe/get/list calls, no
`Tag*`/`Untag*`, delete, STS assumption, or custody mutation/request. A separate
custody helper, not the observer, persists signed observer results. Normal
creation uses exact required creation tags, including EC2 `CreateTags`, IAM
`TagRole`, and `TagInstanceProfile`, only for the just-created recorded resource.
Cleanup may remove only exact recorded campaign/intent tags when the declared
resource deletion requires it (`DeleteTags`, `UntagRole`,
`UntagInstanceProfile`); it has no generic retagging or create authority. The
recovery/fence role may mutate only the exact normal/backend trust documents and
the named deny-all policies needed to attach, read back, retain, and finally
remove that fence. It has no tagging, graph, backend-data, or other IAM mutation.
The generated permission closure must reject any contrary action in these rows.

**Accounting and compatibility mechanics.** The three AWS categories are
...
preauthorization for the pinned backend/saved-plan compatibility design. It
...
6. PR publication and required exact-head Quality, secret, images, and applicable root checks;
7. separately authorized protected product/KVM feedback runs;
8. candidate publication and durable readback;
9. separately reviewed no-mint full validation plus regenerated readiness;
10. recovery/residue validation and two independent audits;
11. final full validation and deterministic regeneration;
12. exact-tree reviews and changed-since-review checks;
13. separately authorized authenticated control-account planning and native
    backend compatibility gate, with no workload credential or resource effect;
14. separately reviewed pre-dispatch review of the exact candidate, external
    custody, normal/backend/cleanup role split, fence proof, v3 evidence
    boundaries, and unchanged denial inventory;
15. retirement/freeze decision; then
16. H.
