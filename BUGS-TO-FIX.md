...
Protected R was documentation/governance only. The later R3 and R4 corrections and R5 governance-only successor authorized below are limited to the production-approval Cosign filesystem/runtime boundary, an exact non-authorizing signing diagnostic, synchronized tests, additive retirement records, and deterministic readiness evidence. None changes H, G, Q, or the qualified implementation.
