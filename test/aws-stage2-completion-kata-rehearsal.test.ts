...
const readinessWrapper = readFileSync("deploy/aws-feasibility/remote/run-stage2-completion-readiness-rehearsal.sh", "utf8");

test("authentic full and readiness rehearsals enter production routes but cannot mint", () => {
  assert.match(coordinator, /def _run_fixed_full_rehearsal\(\):[\s\S]*?_run_cycle\(cycle_evidence\._fixed_full_route\(\),[\s\S]*?False\)/u);
  assert.match(coordinator, /def _run_fixed_readiness_rehearsal\(\):[\s\S]*?_run_cycle\(cycle_evidence\._fixed_readiness_route\(\),[\s\S]*?False\)/u);
