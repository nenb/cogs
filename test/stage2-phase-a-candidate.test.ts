...
test("runtime-discovery workflow has the exact PR 230 one-shot guard and cleanup order", async () => {
  ...
  assert.match(
    nativeJob,
    /"--securebits","\+noroot,\+noroot_locked"[\s\S]*"--bounding-set=-all","--inh-caps=-all","--ambient-caps=-all"[\s\S]*"--keep-groups","--no-new-privs"[\s\S]*"\/usr\/bin\/timeout","--signal=KILL","240","\/usr\/bin\/python3","-I","-B","-c",seccomp/u,
  );
