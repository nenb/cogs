...
runtime.update(version='cogs.runtime-qualification/v1',marker=ns['MARKER'],source_revision=revision,
 source_set_sha256=source_digest,closure_sha256='3'*64,
...
value={'version':ns['RESULT_VERSION'],'source_revision':revision,'source_set_sha256':source_digest,
 'closure_sha256':'3'*64,'tools':[tool('gzip','6'*64,10),tool('zstd','7'*64,13)],'runtime':runtime}
rows=ns['qualify'](value,revision,source_digest)
assert rows == value['tools'] and all(row['seal_mask']==63 for row in rows)
for mutation in ('mask15','equal-wrong','source-sealed','size','mapping','object','tool-substitution','revision','false-fact'):
...
test("Job B safe adapter cannot replace the production owner with a completed mock", () => {
  const result = portable(`
events=[]
...
assert events == [('begin','B'),('operation','B'),('settle',),('publish','stopped-before-owner')]
`);
  assert.equal(result.status, 0, result.stderr);
});
