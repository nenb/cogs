...
test("runtime contract rejects every mutable path, profile, protocol, bound, and unknown field", () => {
  ...
  const cases: Array<(runtime: Record<string, any>) => void> = [
    ...
    (value) => (value.paths.skill_snapshot_receipt = "/tmp/receipt"),
    (value) => (value.paths.skill_snapshot_control_socket = "/tmp/control.sock"),
    (value) => delete value.paths.skill_snapshot_receipt,
    (value) => delete value.paths.skill_snapshot_control_socket,
