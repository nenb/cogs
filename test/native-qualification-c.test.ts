...
test("Job C cannot fabricate descriptor checks or reread common cleanup", () => {
  const harness = `
import runpy,sys,types
module=runpy.run_path(${JSON.stringify(path)},run_name='job_c_test')
events=[]
class Bomb:
...
class NativeSession:
 @staticmethod
 def begin(job,driver): assert (job,driver)==('C',${JSON.stringify(path)});return session
common=types.SimpleNamespace(NativeSession=NativeSession,ReportCandidate=Candidate)
assert module['_run'](common)==0
assert events==[('operation','C'),('settle',),('publish',None,None,None)]
...
ops=Ops();real_session=real_common.NativeSession._begin_with_ops(types.SimpleNamespace(job='C'),ops,Cust())
...
selected=[]
assert module['_dispatch'](['--workflow-bound'],lambda common:selected.append(common) or 5,lambda:'common')==5
...
`;
  ...
  assert.doesNotMatch(source, /production_checks|metadata|close_range|inheritance_exact|source_set_sha256/u);
  assert.doesNotMatch(source, /fork\(|pidfd|waitid|waitpid|\/proc\//u);
