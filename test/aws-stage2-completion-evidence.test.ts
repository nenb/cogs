...
import { renderAwsStage2CompletionReport } from "../scripts/render-aws-stage2-completion-report.ts";
import {
  CompletionEvidenceValidationError,
  parseAwsStage2CompletionEvidence,
  validateAwsStage2CompletionEvidence,
} from "../scripts/validate-aws-stage2-completion-evidence.ts";
...
function fixture(): Record<string, any> {
  ...
  return {
    ...
    cost: {
      ...
      aggregate_duration_ns: cycles.reduce((sum, cycle) => sum + cycle.duration_ns, 0),
...
test("closed v1 accepts exact seven cycles and renderer accepts only its validator token", () => {
  const value = fixture();
...
test("eight fresh zeros, all private freshness domains, deadlines, and integer costs are recomputed", () => {
  ...
  reject((value) => {
    value.cost.aggregate_duration_ns += 1;
  }, "duration aggregate");
  reject((value) => {
...
test("historical, local, fake, partial, extra, and every forbidden public string class are rejected", () => {
  for (const [version, authority] of [
    ...
    ["cogs.aws-stage2-completion-evidence/v1", "stage4-readiness"],
  ])
    reject((value) => {
