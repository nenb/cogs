...
test("dedicated workflow is manual, same-repository, and exact reviewed H/G", () => {
  ...
  assert.match(guard, /REVIEWED_IMPLEMENTATION_HEAD = "892c3fc44e37d74792fe552107839b920ea98e8e"/u);
  assert.match(
    guard,
    /REVIEWED_IMPLEMENTATION_MANIFEST_SHA256 = "7f159309740386b0510f72948d8b39038c4470a93f0e3bbd816d0cf437fdad71"/u,
  );
  assert.match(guard, /REVIEWED_CONTROL_SHA256 = "30411c84bdf78128300524dd2e72c239123f0fc79608ee1976013232953fb1d5"/u);
  assert.match(guard, /REVIEWED_WORKFLOW_SHA256 = "22290e474e6325766d64f47d87d67dec713743b93fc8df1476c83de37e9183c3"/u);
  assert.match(
    guard,
...
test("attempt-one stable admission is the first step and precedes unauthenticated H/G acquisition", () => {
  ...
  assert.match(workflow.slice(admission, control), /len\(runs\) == 9/u);
...
test("fixed phase bounds preserve recovery, independent residue, and publication reserve", () => {
  ...
  assert.equal(job, 145);
  assert.equal(total, 141);
  assert.equal(total - stepTimeout("Admit the stable first-created dispatch before every source effect"), 140);
  ...
  assert.match(workflow, /one 8,700-second envelope/u);
});
