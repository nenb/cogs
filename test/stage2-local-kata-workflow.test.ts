...
test("dedicated workflow is manual, same-repository, and exact reviewed H/G", () => {
  ...
  assert.match(guard, /REVIEWED_WORKFLOW_SHA256 = "28b9e428e2a97568a5b2992ee57de06158e80ff0d4c61f2e1beaab0475aa62d4"/u);
  assert.match(
    guard,
...
test("attempt-one stable admission is the first step and precedes unauthenticated H/G acquisition", () => {
  ...
  assert.match(workflow.slice(admission, control), /len\(runs\) == 15/u);
