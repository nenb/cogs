...
test("dedicated workflow is manual, same-repository, and exact reviewed H/G", () => {
  ...
  assert.match(guard, /REVIEWED_WORKFLOW_SHA256 = "80eecba5618c4c402c4e6381de6ef4daab09133ce5e5925e8bda2169ad26b8ba"/u);
  assert.match(
    guard,
...
test("attempt-one stable admission is the first step and precedes unauthenticated H/G acquisition", () => {
  ...
  assert.match(workflow.slice(admission, control), /len\(runs\) == 13/u);
