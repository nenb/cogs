...
const runPython = (program: string) => {
  const result = spawnSync("python3", ["-I", "-B", "-c", program], { encoding: "utf8", timeout: 90_000 });
...
assert 'docs/adr/0341-row4-third-minimal-correction-batch.md' in tasks[1]['paths']
...
assert 'deploy/aws-feasibility/check-plan.py' in local
assert 'deploy/aws-feasibility/main.tf' in local
...
test("ADR0369 retires failed R planning and preserves separate authority", () => {
  const adr = readFileSync("docs/adr/0369-retire-failed-planning-and-align-expiry.md", "utf8").replace(/\s+/gu, " ");
  for (const text of [
    "727636d61207903744aba52cb6a2325e92f13f8b",
    "36096732289",
    "stage2-production-planner: owner.failed",
    "cannot be retried, stitched, resumed, salvaged, or reinterpreted",
    "strict eight-hour maximum to a strict ten-hour maximum",
    "25,062 lines and 21,500,000 bytes",
    "43,062 lines and 29,500,000 bytes",
    "PRODUCT_TEST_PENDING_READINESS_REGENERATIONS=0",
    "tracked-file limit only from 1,568 to 1,569",
    "source revision grants no credential, IAM, planning, approval, apply, production, closure, or Stage 4 authority",
    "authorize-seven-stage2-production-cycles",
  ])
    assert.ok(adr.includes(text), text);
  assert.ok(
...
assert m['PRODUCT_TEST_PENDING_READINESS_REGENERATIONS'] == 0
...
test("ADR0360 retires the image-rejected generation and authenticates assigned images", () => {
  const adr = readFileSync("docs/adr/0360-retire-runner-image-rejected-generation.md", "utf8").replace(/\s+/gu, " ");
  for (const text of [
    "21848f84f01d42f28ce2f6f2a177bfe62af8fc58",
    "35716917245",
    "35732877526",
    "35733919360",
    "zero artifacts",
    ...
    "tag-ref commit",
    "All nineteen pre-effect selectors",
    "execution envelope V4",
    "cycle-status V3",
    "pre-aws-package-v6.json",
    "one wholly fresh protected H/G/Q",
    "Q's seven GitHub-hosted KVM cycles",
    "R's later seven sequential AWS",
    "global headroom",
    "from 2,150 to 3,800",
    "from 1,500 to 3,150",
  ])
...
test("ADR0364 retires stale Q and requires complete static handoff", () => {
  const path = "docs/adr/0364-retire-stale-Q-bindings-and-require-complete-static-handoff.md";
  const adr = readFileSync(path, "utf8").replace(/\s+/gu, " ");
  for (const text of [
    "35946454149",
    "f75d3f09990de4635cc3890efe0f5f6e783312bd",
    "448989760dc077d27ea9b4bba5e9cbe50007f63b42cd93d5c52f6129c5fdf4e4",
    "config/stage2-retired-revisions-v7.json",
    ...
    assert.ok(adr.includes(text), text);
  assert.ok(readFileSync("docs/adr/README.md", "utf8").includes(`[0364](${path.slice(9)})`));
});
