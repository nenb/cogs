...
assert highs==dict(route=1800,revocation=3000,relay=1800,lifecycle=6700,completion=2300,integration=2900)
...
assert new==dict(route=1,revocation=0,relay=0,lifecycle=4,completion=3,integration=27)
assert sum(new.values())==35 and sum(x['total'] for x in forecasts.values())==2570000
...
assert len(set(paths)-baseline)==35
for current in ('docs/adr/0310-authorize-openbao-recognition-correction.md','docs/adr/0311-reallocate-integrated-post-H-closure.md','docs/adr/0312-reallocate-final-observer-closure.md','docs/adr/0313-authorize-final-hostile-corrections-and-stage2-scope.md','docs/adr/0314-raise-final-hostile-integration-ceilings.md','docs/adr/0315-authorize-final-async-transport-ownership.md'):
 assert paths[current]=='integration' and Path(current).is_file()
for number,name in ((316,'freeze-remediated-H-and-authorize-control'),(317,'establish-remediated-Q-and-authorize-qualification')):
