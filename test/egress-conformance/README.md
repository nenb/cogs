...
Candidate-specific immutable configuration and external-process lifecycle adapters live under `proxy-adapters/`. The pinned Envoy candidate uses native gRPC `ext_authz`; the pinned mitmproxy alternate requires a measured Python addon over parsed flows. Both validate capability and authorization/audit hooks fail closed, expose no administration endpoint, and run the same guest-root candidate smoke. Their security-labelled CI reports remain functional-only and stub-aware. `cases/stage-1.ts` is the immutable shared black-box matrix. It covers capability and route isolation, HTTP/1 and HTTP/2 ambiguity, credential overwrite and absence, redacted OTLP/intent/completion behavior, injected failures, revocation, process drain, capability rotation, and certificate replacement. Each deny-or-safe-normalization case names a passing positive control. Both candidates execute the unchanged matrix from root in the insecure guest; results remain functional-only and stub-aware, not proxy-selection evidence by themselves.
