...
Guest `Authorization`, `Proxy-Authorization`, and configured API-key headers are removed at the route and the configured bearer, Basic, or API-key value is overwritten. Capability validation and per-request intent authorization use Envoy `ext_authz` with `failure_mode_allow: false`; authorization and audit outages therefore deny. Envoy's raw HTTP ext-authz transport otherwise forwards a CONNECT method and interprets a 200 response as a tunnel, so two fixed Lua header filters temporarily present that authorization subrequest as GET and restore CONNECT before routing. They remove any guest marker first, do not parse payloads or targets, and never alter non-CONNECT methods. The inner structured completion log contains only intent ID, route ID, status, and bounded latency. It contains no path, query, body, header value, capability, or credential.
