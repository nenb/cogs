...
Guest `Authorization`, `Proxy-Authorization`, and configured API-key headers are removed at the route and the configured bearer, Basic, or API-key value is overwritten. The exact proxy-capability wire value is `Basic base64(ASCII("cogs:" + capability))`: one value and one separating space, with fixed username `cogs`, the unchanged base64url-without-padding capability (32 through 128 characters) as password, and canonical padded RFC 4648 Base64. Raw, Bearer, empty, malformed, non-canonical, duplicate, and combined values are denied. The password is checked through a keyed digest and constant-time digest comparison, never logged, and cleared from temporary mutable buffers. Capability validation and per-request intent authorization use Envoy's native v3 gRPC `ext_authz` contract with `failure_mode_allow: false`; authorization and audit outages therefore deny. Route-specific context extensions carry only opaque trusted case/session/route metadata, while the request carries the encoded proxy capability for keyed comparison. No Lua or custom HTTP request parser is used; Envoy remains the sole HTTP/1.1, HTTP/2, CONNECT, and TLS parser. The inner structured completion log consumes the returned opaque intent ID as dynamic metadata and contains only intent ID, route ID, status, and bounded latency. It contains no path, query, body, header value, capability, or credential.
