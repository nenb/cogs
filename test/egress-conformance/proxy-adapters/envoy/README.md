...
Guest `Authorization`, `Proxy-Authorization`, and configured API-key headers are removed at the route and the configured bearer, Basic, or API-key value is overwritten. Capability validation and per-request intent authorization use Envoy `ext_authz` with `failure_mode_allow: false`; authorization and audit outages therefore deny. The inner structured completion log contains only intent ID, route ID, status, and bounded latency. It contains no path, query, body, header value, capability, or credential.
