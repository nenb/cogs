...
The latest upstream image still has six unique fixed HIGH finding identifiers as of the 2026-07-15 repository Trivy scan. The expiring `.trivyignore-mitmproxy` exception was removed instead of renewed for Stage 3. This adapter remains historical Stage 1 comparison code only: it is not scanned or allowed as an active selected CI image, not the Stage 3 implementation path, and not a release fallback. See `docs/security-evidence/mitmproxy-12.2.3-retirement.md`.
