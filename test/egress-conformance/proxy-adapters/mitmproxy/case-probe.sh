...
for name in COGS_MITMPROXY_GUEST_PROXY COGS_MITMPROXY_TARGET COGS_MITMPROXY_PROXY_CA COGS_MITMPROXY_CAPABILITY COGS_MITMPROXY_EXPECT; do
  [[ -n "${!name:-}" ]] || { printf '{"passed":false,"diagnosticsRedacted":"probe input is absent"}\n'; exit 0; }
done
if [[ ! "$COGS_MITMPROXY_GUEST_PROXY" =~ ^http://host\.docker\.internal:[0-9]{1,5}$ \
    || ! "$COGS_MITMPROXY_TARGET" =~ ^https://localhost:[0-9]{1,5}/protected/header$ \
    || ! "$COGS_MITMPROXY_CAPABILITY" =~ ^[A-Za-z0-9._-]{16,256}$ \
    || ( "$COGS_MITMPROXY_EXPECT" != allow && "$COGS_MITMPROXY_EXPECT" != deny ) ]]; then
  printf '{"passed":false,"diagnosticsRedacted":"probe input is invalid"}\n'
  exit 0
fi

if ! COGS_HTTP_PROXY="$COGS_MITMPROXY_GUEST_PROXY" \
  COGS_HTTPS_PROXY="$COGS_MITMPROXY_GUEST_PROXY" \
  COGS_PUBLIC_CA_FILE="$COGS_MITMPROXY_PROXY_CA" \
...
result=$(ssh "${ssh_options[@]}" root@127.0.0.1 \
  bash -s -- "$COGS_MITMPROXY_EXPECT" "$COGS_MITMPROXY_TARGET" "$COGS_MITMPROXY_CAPABILITY" 2>>"$log" <<'REMOTE'
set -euo pipefail
expect=$1
target=$2
capability=$3
