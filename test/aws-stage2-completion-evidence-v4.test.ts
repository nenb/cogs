...
} from "../scripts/render-aws-stage2-completion-report-v4.ts";
...
function cliFailure(category: string) {
  return { code: 2, stdout: "", stderr: `completion-evidence-v4: ${category}\n` };
}
...
test("CLI input read is bounded, no-follow, regular, single-link, and exact", () => {
  const directory = fs.mkdtempSync(join(tmpdir(), "cogs-evidence-v4-"));
  try {
...
test("test-only canonical v4 projection validates and renderer requires its validator token", () => {
...
test("every available binding rejects an independent schema-valid contradictory digest", () => {
  ...
  const paths: string[][] = [
    ...
    ...[
      ...
      "continuation_admission_commitment",
      "cycle3_zero_commitment",
    ].map((key) => ["custody", "handoff", key]),
  ];
...
test("CLI failures have exact bounded allowlisted stderr and exit 2, without subprocesses", () => {
  ...
  assert.deepEqual(completionEvidenceCliDiagnostic(new Error(sensitive)), "completion-evidence-v4: internal\n");
  assert.equal(completionEvidenceCliDiagnostic({ category: sensitive }), "completion-evidence-v4: internal\n");
  assert.equal(
    completionEvidenceCliDiagnostic(
      Object.assign(new CompletionEvidenceValidationError(sensitive), { category: sensitive }),
    ),
    "completion-evidence-v4: internal\n",
  );
  assert.deepEqual(captureCli(runAwsStage2CompletionEvidenceCli, [cliEvidence]), {
    code: 0,
    stdout: "Validated closed AWS Stage 2 completion evidence v4.\n",
    stderr: "",
  });
  let diagnostic = "";
  assert.equal(
  ...
  assert.equal(diagnostic, "completion-evidence-v4: internal\n");
  assert.equal(
    runAwsStage2CompletionEvidenceCli([], {
      stdout: () => {},
      stderr: () => {
...
test("renderer holds and verifies output directory/file custody across writes and failures", (t) => {
  const directory = fs.realpathSync(fs.mkdtempSync(join(tmpdir(), "cogs-report-v4-")));
