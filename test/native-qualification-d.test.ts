...
test("Job D retains real identity-bound bounded lifecycle primitives", () => {
  for (const token of ["PR_SET_PDEATHSIG", "pidfd_open", "pidfd_send_signal", "/proc/{pid}/stat", "os.setsid", "os.getppid", "SIGTERM", "SIGKILL", "waitid", "WEXITED", "--native"]) {
    assert.ok(source.includes(token), token);
  }
  for (const check of ["before_release_death", "after_release_death", "starttime_revalidated", "process_group_owned", "all_reaped"]) {
